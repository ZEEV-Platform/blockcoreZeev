    ...
    public class ZEEVBlockMerkleRootRule : IntegrityValidationConsensusRule
    {
        ...
        public static uint256 BlockMerkleRoot(Block block, out bool mutated)
        {
            ...
            foreach (ZEEVTransaction tx in block.Transactions)
            {
                ...
                if (txClone.IsCoinBase)
                {
                    var ops = txClone.Inputs[0].ScriptSig.ToOps();
                    ...
                    var newScript = new Script(ops);
                    var bytesNewScript = newScript.ToBytes();
                    var bytesScript = txClone.Inputs[0].ScriptSig.ToBytes();

                    var messageLength = bytesScript.Length - bytesNewScript.Length - 8; //8 bytes for extranonce
                    
                    if (bytesScript.Length > (bytesNewScript.Length + 8))
                    {
                        var message = new byte[messageLength];
                        Buffer.BlockCopy(bytesScript, bytesNewScript.Length + 8, message, 0, messageLength);

                        bytesNewScript = bytesNewScript.Concat(message);
                    }
