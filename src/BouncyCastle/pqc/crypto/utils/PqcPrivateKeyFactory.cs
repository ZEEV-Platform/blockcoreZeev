...
#pragma warning restore CS0618 // Type or member is obsolete
                ...
                algOid.Equals(BCObjectIdentifiers.falcon_1024))
            {
                ...
                var keySeq = (Asn1Sequence)DerSequence.FromByteArray(DerOctetString.GetInstance(keyEnc[2]).GetOctets());

                return new FalconPrivateKeyParameters(spParams,
                    Asn1OctetString.GetInstance(keySeq[1]).GetOctets(),
                    Asn1OctetString.GetInstance(keySeq[2]).GetOctets(),
                    Asn1OctetString.GetInstance(keySeq[3]).GetOctets(),
